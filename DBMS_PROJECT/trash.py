...
def register_clicked():
    ...
    reg_username_label = CTkLabel(register_window, text="Username:")
    reg_username_label.pack(pady=5)

    reg_username_entry = CTkEntry(register_window)
    reg_username_entry.pack(pady=5)

    reg_password_label = CTkLabel(register_window, text="Password:")
    reg_password_label.pack(pady=5)

    reg_password_entry = CTkEntry(register_window, show="*")
    reg_password_entry.pack(pady=5)

    phone_number_label = CTkLabel(register_window, text="Phone Number:")
    phone_number_label.pack(pady=5)

    phone_number_entry = CTkEntry(register_window)
    phone_number_entry.pack(pady=5)

    address_label = CTkLabel(register_window, text="Address:")
    address_label.pack(pady=5)

    address_entry = CTkEntry(register_window)
    address_entry.pack(pady=5)

    img = Image.open("pic1.png")

    register_button = CTkButton(register_window, text="Register", corner_radius=32, image=CTkImage(dark_image=img), command=lambda: register(
...
def open_booking_window():
    ...
    booking_window = CTkToplevel(root)
...
def book_ticket(train_name, cost, ticket_id):
    try:
...
''', {'passenger_id': current_passenger_id, 'ticket_id_fin': ticket_id_fin})
            ...
            connection.commit()
            messagebox.showinfo("Booking Successful", f"Ticket booked for Train {train_name}. Cost: {cost}")
        else:
            messagebox.showinfo(
...
login_button = CTkButton(root, text="Login", corner_radius=32,
                         fg_color="transparent", command=login_clicked)
login_button.pack(pady=10)

register_button = CTkButton(root, text="Register", corner_radius=32,
                            fg_color="transparent", command=register_clicked)
register_button.pack(pady=10)
